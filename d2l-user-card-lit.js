...
export class UserTile extends LitElement {
	...
	render() {
		return html`
			<d2l-card text="${this._getA11YTitleString(this.text, this.name)}" ?loading="${this._placeholders}" href="javascript:void(0);">
				<d2l-card-loading-shimmer ?loading="${this._placeholders}" slot="header">
					<div class="user-tile-background" style="${this._getBackgroundStyle(this.background, this.backgroundColor)}"></div>
				</d2l-card-loading-shimmer>
				<div slot="badge" class="user-tile-avatar" aria-hidden="true">
				${!this._hideIconPlaceholder(this.icon, this._placeholders) ? html`
				...
				<div class="d2l-body-compact user-tile-information-wrapper" slot="content">
					<h3 class="user-tile-name" ?hidden="${!this._hideNamePlaceholder(this._placeholders, this.name)}">${this.name}</h3>
					<p class="user-tile-name text-placeholder" ?hidden="${this._hideNamePlaceholder(this._placeholders, this.name)}"></p>
	...
	_hideIconPlaceholder(icon, placeholders) {
		return !placeholders && !!icon;
	}

	_hideNamePlaceholder(placeholders, name) {
	...
	_getA11YTitleString(text, name) {
		return `${text || name}`;
	}

	_getBackgroundStyle(background, backgroundColor) {
		if (background) {
...
customElements.define('d2l-user-card-lit', UserTile);
